...
def test_get_predict_list():
    """
    test if can return prediction list 
    """
    with app.test_client() as c:
...
def test_predict_house_price():
    """
    test if can run predict method 
    """
    with app.test_client()as c:
...
def test_predict_house_price_with_input():
    """
    test if can run predict-with-input (json) method 
    """
    ...
    with app.test_client()as c:
        response = c.post('/REST/api/v1.0/predict_with_input', headers=headers, data=data)
        assert response.status_code == 201
